...
export function MenuHeader({ restaurant }: MenuHeaderProps) {
  return (
    <div className="bg-card/40 border-b border-border/30 px-4 py-4">
      <div className="container mx-auto max-w-2xl flex items-center gap-3">
        ...
        <div className="shrink-0">
          {restaurant.logo_url ? (
            ...
            <div className="w-14 h-14 rounded-full bg-primary/15 border-2 border-primary/30 flex items-center justify-center">
              <span className="font-serif text-primary text-xl">{restaurant.name.charAt(0)}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-lg font-bold text-foreground leading-tight">{restaurant.name}</h1>
            <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-green-500/15 text-green-400 border border-green-500/25">
        ...
        {/* Social icons */}
        {(restaurant.maps_url || restaurant.instagram_url || restaurant.facebook_url) && (
          <div className="flex items-center gap-1 shrink-0">
            ...
            {restaurant.instagram_url && (
              <a href={restaurant.instagram_url} target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-full bg-secondary/60 flex items-center justify-center text-muted-foreground hover:text-pink-400 hover:bg-secondary transition-colors">
                <InstagramIcon />
              </a>
            )}
            {restaurant.facebook_url && (
              <a href={restaurant.facebook_url} target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-full bg-secondary/60 flex items-center justify-center text-muted-foreground hover:text-blue-400 hover:bg-secondary transition-colors">
                <FacebookIcon />
              </a>
            )}
          </div>
